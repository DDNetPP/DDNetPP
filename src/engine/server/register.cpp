...
void CRegister::RegisterUpdate(int Nettype)
{
	...
	if(m_RegisterState == REGISTERSTATE_START)
	{
		...
		m_pConsole->Print(g_Config.m_SvMasterServerLogs, m_pName, "refreshing ip addresses");
	}
	else if(m_RegisterState == REGISTERSTATE_UPDATE_ADDRS)
	{
		...
		if(!m_pMasterServer->IsRefreshing())
		{
			...
			m_pConsole->Print(g_Config.m_SvMasterServerLogs, m_pName, "fetching server counts");
			m_LastTokenRequest = Now;
	...
	else if(m_RegisterState == REGISTERSTATE_QUERY_COUNT)
	{
		...
		if(Left == 0 || Now > m_RegisterStateStart + Freq * 5)
		{
			...
			{
				m_pConsole->Print(g_Config.m_SvMasterServerLogs, m_pName, "WARNING: No master servers. Retrying in 60 seconds");
			...
			else
			{
				...
				m_pConsole->Print(g_Config.m_SvMasterServerLogs, m_pName, aBuf);
	...
	else if(m_RegisterState == REGISTERSTATE_HEARTBEAT)
	{
		...
		{
			m_pConsole->Print(g_Config.m_SvMasterServerLogs, m_pName, "WARNING: Master server is not responding, switching master");
	...
	{
		if(m_RegisterFirst)
			m_pConsole->Print(g_Config.m_SvMasterServerLogs, m_pName, "server registered");
...
int CRegister::RegisterProcessPacket(CNetChunk *pPacket, SECURITY_TOKEN ResponseToken)
{
	...
	{
		if(m_RegisterFirst)
			m_pConsole->Print(g_Config.m_SvMasterServerLogs, m_pName, "no firewall/nat problems detected");
	...
	{
		m_pConsole->Print(g_Config.m_SvMasterServerLogs, m_pName, "ERROR: the master server reports that clients can not connect to this server.");
		char aBuf[256];
