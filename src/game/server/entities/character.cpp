...
void CCharacter::Tick()
{
	...
	{
		if(m_Core.m_HookedPlayer != -1 && GameServer()->m_apPlayers[m_Core.m_HookedPlayer]->GetTeam() != TEAM_SPECTATORS && !m_Core.HookFlag())
		{
...
void CCharacter::Die(int Killer, int Weapon, bool SendKillMsg, bool fngscore)
{
	...
	else if(SendKillMsg && (Team() == TEAM_FLOCK || Teams()->Count(Team()) == 1 || Teams()->GetTeamState(Team()) == CGameTeams::TEAMSTATE_OPEN || Teams()->TeamLocked(Team()) == false))
	{
