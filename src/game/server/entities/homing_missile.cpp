...
CHomingMissile::CHomingMissile(CGameWorld *pGameWorld, int Lifetime, int Owner)
...
bool CHomingMissile::Hit(CCharacter* pHitTarget)
{
	...
	if(distance(m_Pos, TargetPos) < 40.f)
	{
		GameServer()->CreateExplosion(TargetPos, m_Owner, WEAPON_GRENADE, true, 0, GameServer()->GetPlayerChar(m_Owner)->Teams()->TeamMask(0));
		GameServer()->CreateSound(m_Pos, SOUND_GRENADE_EXPLODE, GameServer()->GetPlayerChar(m_Owner)->Teams()->TeamMask(0));
