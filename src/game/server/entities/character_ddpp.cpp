...
vec2 CCharacter::GetSpawnPositionDDPP()
{
	// by default we spawn on the spawn set by ddnet
	vec2 Pos = m_Pos;
	...
	{
		Pos.x = g_Config.m_SvSuperSpawnX * 32;
		Pos.y = g_Config.m_SvSuperSpawnY * 32;
	}
	...
	else if(m_pPlayer->m_IsNoboSpawn)
	{
		...
		if(m_pPlayer->m_NoboSpawnStop > Server()->Tick())
		{
			...
			Pos.x = g_Config.m_SvNoboSpawnX * 32;
			Pos.y = g_Config.m_SvNoboSpawnY * 32;
		}
		else
		{
...
void CCharacter::PreSpawnDDPP()
{
	...
	// set the spawn position here

	vec2 Pos = GetSpawnPositionDDPP();
	m_Pos = Pos;
	m_Core.m_Pos = Pos;
}
...
{
	// do not set spawn positions in this method use PreSpawnDDPP instead
	// otherwise telporters on the spawn tile can teleport you out of jail
	// https://github.com/DDNetPP/DDNetPP/issues/409
