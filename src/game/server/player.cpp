...
void CPlayer::Snap(int SnappingClient)
{
	...
	if(pSnapping)
	{
		if(pSnapping->GetTeam() == TEAM_SPECTATORS || !pSnapping->m_IsSurvivaling || !pSnapping->m_IsSurvivalAlive) //could add a bool is dead here too to activate name again
		{
			m_ShowName = true;
		}
	}
	...
	if(!DDPPSnapChangeSkin(&ClientInfo))
	{
	...
	else
	{
		...
		// ddnet++
		PlayerInfo.m_PlayerFlags = GameServer()->m_pController->SnapPlayerFlags7(SnappingClient, this, PlayerInfo.m_PlayerFlags);
