...
void CPlayer::CalcExp()
{
	...
	//										xp diff
	int BaseXp = 5000; // Base experience required for level 0

	if (GetLevel() == 0) { // If the player is at level 0 (starting level)
		m_neededxp = BaseXp; // Set required XP to 5000 for the next level
	} else {
		double TotalXP = 0; // Variable to store the total required XP

		for (int i = -1; i < GetLevel(); i++) // Loop through all levels starting from -1
		{
			double Growth = 1.15; // Experience growth multiplier per level
			TotalXP += BaseXp * pow(Growth, i); // Calculate XP using geometric progression formula
		}

		m_neededxp = static_cast<int64_t>(TotalXP); // Set the calculated XP value
	}
